...
# --- 4. SỬA LỖI CẤU HÌNH TRANG (THÊM DẤU PHẨY) ---
st.set_page_config(
...
/* --- CSS CHO GIAO DIỆN THÂN THIỆN (Giữ nguyên) --- */
.assistant-card {
...
/* Nút này là class, không phải global, giữ lại */
.big-friendly-button {
...
/* --- 6. THÊM CSS TƯƠNG THÍCH ĐIỆN THOẠI --- */
@media (max-width: 900px) {
    ...
    .exercise-card, .inclusive-instruction, .progress-container {
        padding: 1rem 0.8rem;
        font-size: 1rem;
        max-width: 96vw;
    }
...
# --- HÀM TEXT-TO-SPEECH CẢI TIẾN (Giữ nguyên) ---
...
def gtts_with_diagnostics(text):
    if not GTTS_AVAILABLE: return None, "gTTS không có sẵn trong hệ thống"
...
def get_error_message(error_code):
    ...
    if error_code.startswith("unknown_error:"): return "🔍 Có lỗi không xác định xảy ra."
